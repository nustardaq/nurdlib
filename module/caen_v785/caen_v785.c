...
static void	caen_v785_cmvlc_init(struct Module *,
    struct cmvlc_stackcmdbuf *, int);
static uint32_t caen_v785_cmvlc_fetch_dt(struct Module *,
    const uint32_t *, uint32_t, uint32_t *);
static uint32_t caen_v785_cmvlc_fetch(struct Crate *, struct
...
#if NCONF_mMAP_bCMVLC
void
caen_v785_cmvlc_init(struct Module *a_module,
    struct cmvlc_stackcmdbuf *a_stack, int a_dt)
{
	struct CaenV785Module *v785;

	MODULE_CAST(KW_CAEN_V785, v785, a_module);
	caen_v7nn_cmvlc_init(&v785->v7nn, a_stack, a_dt);
}

uint32_t
caen_v785_cmvlc_fetch_dt(struct Module *a_module,
    const uint32_t *a_in_buffer, uint32_t a_in_remain, uint32_t *a_in_used)
{
	struct CaenV785Module *v785;

	MODULE_CAST(KW_CAEN_V785, v785, a_module);
	return caen_v7nn_cmvlc_fetch_dt(&v785->v7nn,
	    a_in_buffer, a_in_remain, a_in_used);
}

uint32_t
caen_v785_cmvlc_fetch(struct Crate *a_crate,
    ...
    const uint32_t *a_in_buffer, uint32_t a_in_remain, uint32_t *a_in_used)
{
	...
	MODULE_CAST(KW_CAEN_V785, v785, a_module);
	return caen_v7nn_cmvlc_fetch(a_crate, &v785->v7nn,
...
#if NCONF_mMAP_bCMVLC
	MODULE_CALLBACK_BIND(caen_v785, cmvlc_init);
	MODULE_CALLBACK_BIND(caen_v785, cmvlc_fetch_dt);
	MODULE_CALLBACK_BIND(caen_v785, cmvlc_fetch);
#endif
}

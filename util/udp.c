...
udp_address_create(unsigned a_flags, char const *a_hostname, uint16_t a_port)
{
...
#if defined(GETADDRINFO)
	{
		...
		snprintf_(port_str, sizeof port_str, "%u", a_port);
...
udp_client_create(unsigned a_flags, char const *a_hostname, uint16_t a_port)
{
	...
	if (!set_non_blocking(sock)) {
		log_warn(LOGL, "set_non_blocking(%s:%u)", a_hostname, a_port);
		close(sock);
		return NULL;
	}
	CALLOC(client, 1);
...
#if defined(GETADDRINFO)
	{
		...
		for (p = result; NULL != p; p = p->ai_next) {
			...
			if (0 != bind(sock, p->ai_addr, p->ai_addrlen)) {
				log_warn(LOGL, "bind(NULL:%s)", port_str);
				close(sock);
				sock = INVALID_SOCKET;
				continue;
			}
...
#endif
	if (!set_non_blocking(sock)) {
		log_warn(LOGL, "set_non_blocking(NULL:%u)", a_port);
		goto udp_server_create_fail;
	}
	server->socket = sock;
	return server;
